...
class VideoCell: BaseCell {
    ...
    var videoViewModel: VideoViewModel! {
        didSet {
            ...
            setupThumbnailImage()
            setupProfileImage()
            subtitleTextView.text = videoViewModel.subtitleString
            titleLabelHeightConstraint?.constant = videoViewModel.titleLabelHeightConstraint!
        }
    }
    //    var video:Video?
    //    {
    ...
    func setupThumbnailImage() {
        if let thumbnailImageUrl = videoViewModel?.thumbnailImageName {
            thumbnailImageView.loadImageUsingUrlString(urlString: thumbnailImageUrl)
        }
    }
    
    func setupProfileImage() {
        if let profileImageUrl = videoViewModel?.channel?.profileImageName {
